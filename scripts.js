...
async function submitMedical() {
    ...
    const medicalResultDiv = document.getElementById('medical-result');

    try {
        // Show processing message
        medicalResultDiv.innerHTML = "<div class='loading'>Processing your request...</div>";
        ...
        medicalResultDiv.innerHTML = formattedResult;
    } catch (error) {
        medicalResultDiv.innerHTML = `<div class='error'>Error: ${error.message}</div>`;
    }
}


async function submitTravelGuide() {
    const daysofvisit = document.getElementById('travel-days').value;
    const destination = document.getElementById('travel-destination').value;
    const url = "https://flow-api.mira.network/v1/flows/flows/shaswat-suman/travel-guide?version=0.0.1";

    const travelResultDiv = document.getElementById('travel-result');

    try {
        // Show processing message
        travelResultDiv.innerHTML = "<div class='loading'>Processing your request...</div>";

        const response = await fetch(url, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                "miraauthorization": API_KEY, // Replace API_KEY with your actual key
            },
            body: JSON.stringify({
                input: { daysofvisit, destination }
            })
        });
        ...
        travelResultDiv.innerHTML = formattedResult;
    } catch (error) {
        travelResultDiv.innerHTML = `<div class='error'>Error: ${error.message}</div>`;
    }
}
async function locateBloodBank() {
    const location = document.getElementById('bloodbank-location').value;
    const bloodType = document.getElementById('bloodbank-type').value;
    const url = "https://flow-api.mira.network/v1/flows/flows/shivam/blood-bank-locator-ai-flow?version=0.1.0";
    ...
    try {
        const response = await fetch(url, {
            ...
            body: JSON.stringify({
                input: { location, blood_type: bloodType }
            })
        });
        const result = await response.json();

        // Format the result with line separation
        const formattedResult = result.result
            .replace(/\n\n/g, "<hr>") // Replace double line breaks with horizontal rule
            .replace(/\n/g, "<br>");  // Replace single line breaks with <br>

        // Display the formatted result in the bloodbank-result div
        document.getElementById('bloodbank-result').innerHTML = formattedResult;
    } catch (error) {
        document.getElementById('bloodbank-result').innerText = `Error: ${error.message}`;
    }
}
